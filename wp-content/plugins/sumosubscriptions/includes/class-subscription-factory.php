...
class SUMO_Subscription_Factory {
    ...
    public static function get_subscription( $subscription, $user_id = 0 ) {
        ...
        $subscription_id         = $subscription_product_id = 0 ;
                            ...
                            switch ( $subscription_period ) {
                                ...
                                case 'M':
                                    ...
                                    if ( $is_synced && '1' === SUMO_Subscription_Synchronization::$sync_mode && '1' === $subscription_period_value && version_compare( $subscription_version, '9.6', '<=' ) ) {
                                        $current_day = date( 'd', sumo_get_subscription_timestamp() ) ;

                                        if ( $synchronization_period_value > $current_day ) {
                                            $synchronization_period = date( 'm', sumo_get_subscription_timestamp() ) ;
                                        } else {
                                            $synchronization_period = date( 'm', sumo_get_subscription_timestamp( 'next month' ) ) ;
                                        }
                                    }
                                    break ;
                                case 'Y':
