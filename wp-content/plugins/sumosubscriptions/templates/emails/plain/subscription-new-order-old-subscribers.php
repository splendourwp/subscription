...
if ( $admin_template ) {
    echo sprintf( __( 'You have received a Subscription order from your previous subscriber %s %s. The Subscription order is as follows:' , 'sumosubscriptions' ) , sumosubs_get_order_billing_first_name( $order ) , sumosubs_get_order_billing_last_name( $order ) ) . "\n\n" ;
} else {
    echo sprintf( __( 'You have placed another subscription order on %s. The Subscription order is as follows:' , 'sumosubscriptions' ) , get_option( 'blogname' ) ) . "\n\n" ;
}
