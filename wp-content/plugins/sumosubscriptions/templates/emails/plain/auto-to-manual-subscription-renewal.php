...
if ( $order->has_status( 'pending' ) ) {
    ...
    echo sprintf( __( "Your Subscription #%s has been changed to Manual Subscription Renewal because we couldn\'t charge for the Subscription Renewal using your preapproved Payment method. <br>An Invoice has been created for you to renew your Subscription #%s. To pay for this invoice, please use the following link: %s. Please make the payment on or before <b>%s</b>. If payment is not made, Subscription will go to <b>%s</b> status" , 'sumosubscriptions' ) , sumo_get_subscription_number( $post_id ) , sumo_get_subscription_number( $post_id ) , $order->get_checkout_payment_url() , $upcoming_mail_date , $upcoming_mail_status ) . "\n\n" ;
}
