...
<?php if ( $order->has_status( 'pending' ) ) : ?>
    ...
    <p><?php printf( __( 'Your Subscription #%s has been Suspended because Subscription Renewal Payment has not been made. But you can still pay your Subscription by using the Payment Link: %s. <br>Please make the Payment on or before <b>%s</b>. If payment is not made, Subscription will go to <b>%s</b> status' , 'sumosubscriptions' ) , sumo_get_subscription_number( $post_id ) , '<a href="' . esc_url( $order->get_checkout_payment_url() ) . '">' . __( 'pay' , 'sumosubscriptions' ) . '</a>' , $upcoming_mail_date , $upcoming_mail_status ) ; ?></p>
