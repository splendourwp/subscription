...
<p>
    <?php
    printf( __( 'Your Subscription #%s is in Pending Authorization status because we couldn\'t charge your account for Subscription Renewal as your bank have declined the authorization which you have previously given. Please pay using another card or else using any other payment gateway <a href="%s">pay</a>. If payment is not made for the Subscription Renewal by <b>%s</b>. Your Subscription will move to <b>%s</b> status.' , 'sumosubscriptions' ) , sumo_get_subscription_number( $post_id ) , esc_url( $order->get_checkout_payment_url() ) , $upcoming_mail_date , $upcoming_mail_status ) ;
    ?>
</p>
